...
- [Eidos](#eidos)
  ...
  - [2 Core Concepts](#2-core-concepts)
    ...
    - [2.4 Derived Relationships](#24-derived-relationships)
  ...
  - [4 Grammar (ABNF)](#4-grammar-abnf)
    ...
    - [4.11 Derived Relationship Declaration](#411-derived-relationship-declaration)
    - [4.12 Annotations and Doc Comments](#412-annotations-and-doc-comments)
  - [5 Compiler Semantics](#5-compiler-semantics)
    - [5.1 Determinism Analysis](#51-determinism-analysis)
    - [5.2 HTTP Surface Generation](#52-http-surface-generation)
    - [5.3 Role HTTP Surface](#53-role-http-surface)
      - [5.3.1 Participation Objects (R1)](#531-participation-objects-r1)
      - [5.3.2 Role Eligibility Enforcement (R2)](#532-role-eligibility-enforcement-r2)
      - [5.3.3 Roles Facet (R3)](#533-roles-facet-r3)
      - [5.3.4 Role-Disambiguated Projections (R4)](#534-role-disambiguated-projections-r4)
    - [5.4 Derived Relationship HTTP Surface](#54-derived-relationship-http-surface)
      - [5.4.1 Recursive Relationships (R6)](#541-recursive-relationships-r6)
      - [5.4.2 Chained Relationships (R7)](#542-chained-relationships-r7)
      - [5.4.3 Response Representation](#543-response-representation)
    - [5.5 Archetype Composition](#55-archetype-composition)
    - [5.6 Guard Resolution](#56-guard-resolution)
  - [6 Complete Example](#6-complete-example)
    - [6.1 Archetype file: `hr-archetypes.eidos`](#61-archetype-file-hr-archetypeseidos)
    - [6.2 Schema file: `hr.eidos`](#62-schema-file-hreidos)
    - [6.3 Generated HTTP surface (excerpt)](#63-generated-http-surface-excerpt)
  ...
  - [Appendix A — Built-in Archetype Definitions](#appendix-a--built-in-archetype-definitions)
    - [A.1 Immutable](#a1-immutable)
    - [A.2 SoftDeletable](#a2-softdeletable)
    - [A.3 Publishable](#a3-publishable)
    - [A.4 Approvable](#a4-approvable)
    - [A.5 Activatable](#a5-activatable)
    - [A.6 Disableable](#a6-disableable)
    - [A.7 WriteOnce](#a7-writeonce)
    - [A.8 ReadOnly](#a8-readonly)
...
> **Design note:** Archetype composition (e.g., `Disableable + SoftDeletable`) requires merging two state machines into a product automaton. This is supported only for archetypes that declare themselves `composable` and define compatible state namespaces. See §5.5 (TBD).

### 2.4 Derived Relationships

A reified `relationship` is an endurant: it has identity, a URL, properties, and a lifecycle. There are situations where associations between entities exist logically but are **not** independent endurants:

- **Transitive closures** — "all people in the reporting chain above Alice" follows from the direct `Supervises` edges, but the chain itself is not an endurant with a creation date or lifecycle.
- **Chained compositions** — "Person works for Holding (via Employment → Organization → Ownership)" is a derived fact; no separate endurant represents it.

The key distinction: reified relationships have **identity and lifecycle**; derived relationships are **computed, read-only projections** with neither. Eidos models both, but keeps them rigorously separate.

| Kind                     | Identity                | Lifecycle                           | HTTP surface        |
|--------------------------|-------------------------|-------------------------------------|---------------------|
| **Reified relationship** | Yes — has a `_self` URL | Yes — `PUT /_state`, coupling rules | Full CRUD           |
| **Derived relationship** | No                      | No                                  | GET-only, read-only |

Two kinds of derived relationships exist:

- **Recursive** — transitive closure of a self-referential relationship. Declared with the `recursive` keyword, referencing an existing reified relationship and naming the ancestor/descendant roles.
- **Chained** — composition of two or more reified relationships into a multi-hop path. Declared with the `chained` keyword and a `path` block specifying the hop sequence.

Derived relationships never create new endurants. Their entries provide **traceability** back to the underlying reified relationships that justify them.
...
top-decl    =  entity-decl
            /  relationship-decl
            /  recursive-rel-decl       ; derived: transitive closure
            /  chained-rel-decl         ; derived: multi-hop composition
            /  mixin-decl
            /  archetype-decl
...
### 4.11 Derived Relationship Declaration

Derived relationships are computed, read-only associations with no identity or lifecycle. Two kinds:

```abnf
; ── Recursive relationship ────────────────────────────────────
; Transitive closure of a self-referential relationship
recursive-rel-decl = [annotation] "recursive" "relationship" TypeName
                     "on" TypeName                ; base relationship name
                     "{" recursive-body "}"

recursive-body     = *( ws / comment / recursive-member )

recursive-member   = doc-comment
                  /  ancestor-clause
                  /  descendant-clause

ancestor-clause    = "ancestor"   ":" name        ; participant name in base rel
descendant-clause  = "descendant" ":" name        ; participant name in base rel

; Example:
;   recursive relationship ReportsTo on Supervises {
;     ancestor:   supervisor
;     descendant: report
;   }

; ── Chained relationship ──────────────────────────────────────
; Composition of two or more reified relationships
chained-rel-decl   = [annotation] "chained" "relationship" TypeName
                     "{" chained-body "}"

chained-body       = *( ws / comment / chained-member )

chained-member     = doc-comment
                  /  path-block

path-block         = "path" "{" 1*hop-decl "}"

hop-decl           = "via" TypeName "from" name "to" name
                     ; TypeName = relationship; names = participant slots

; Example:
;   chained relationship WorksForHolding {
;     path {
;       via Employment from employee to employer
;       via Ownership  from subsidiary to parent
;     }
;   }
```
...
- For `recursive`: the base relationship must be self-referential (both participants share an entity type), and the named slots must exist.
- For `chained`: each hop references an existing relationship, and the `to` slot of hop N must share an entity type with the `from` slot of hop N+1 (type continuity).

### 4.12 Annotations and Doc Comments
...
### 5.3 Role HTTP Surface

In v0.1 roles are defined in the schema but have no HTTP surface. v0.2 addresses four gaps:

#### 5.3.1 Participation Objects (R1)

A role can declare properties (`employeeId`, `department`). These are neither properties of the participant entity (which exists independent of the relationship) nor of the relationship itself (which is shared across participants). They are properties of the **participation** — this entity, _as it plays this role, in this relationship_.

**Representation.** Each participant slot whose role declares properties is represented as a **participation object** with:

- `_self` — the participant entity's canonical URL (unchanged from v0.1)
- Role properties inline

Participant slots whose roles declare **no** properties remain bare URL strings (backward compatible).

**Default response** (`GET /employments/42`):
...
`employee` is a participation object (the `Employee` role declares properties); `employer` is a bare URL (the `Employer` role declares none).

**Expanded response** (`GET /employments/42?expand=employee,employer`):
...
`?expand` merges the participant entity's reserved fields and properties into the same object. Role properties and entity properties become siblings. The compiler **rejects a role property whose name collides with a property (or reserved field) of the participant entity**.

#### 5.3.2 Role Eligibility Enforcement (R2)

A role may declare `requires:` — an eligibility guard. The enforcement point is **relationship creation**.

On `POST /{relationships}`, for each participant slot whose role declares `requires:`, the server evaluates the guard against the referenced entity. If it fails: **`422 Unprocessable Entity`** (well-formed but domain-invalid).

The compiler emits the predicate name into an `x-eidos-role-requires` OpenAPI extension on the `POST` operation, analogous to `x-eidos-guard`. Predicate resolution is outside the language scope.

#### 5.3.3 Roles Facet (R3)

A role is an **extrinsic classification** an entity acquires by participating in a relationship. The question "what roles does this entity currently play?" should be answerable.

**Generated endpoint.** For every entity that participates in at least one relationship:

```http
GET /{entity}/{key}/_roles
```
...
Each entry names the role, the relationship type, the canonical relationship URL, and the participation's role properties. This is a projection — read-only, no identity.

**Representation placement.** The roles facet is **not** part of the default entity representation (computing it may be expensive). Access options:

- Dedicated `GET /{entity}/{key}/_roles` sub-resource (primary)
- Optional `?include=_roles` on `GET /{entity}/{key}` for inline access

> **Reserved-keyword convention.** Every built-in, fixed URL segment uses a leading underscore (`_state`, `_roles`), so it is visually distinct from designer-defined names and cannot collide with them.

#### 5.3.4 Role-Disambiguated Projections (R4)

Projection collections already encode a role implicitly: `GET /persons/7/employments` means "Employments where person 7 plays the Employee role." This is unambiguous only because the two slots hold _different_ entity types.

When one entity type fills **two** slots (e.g., a self-referential `Supervises` between two `Person`s), the projection is ambiguous. A disambiguator is required:

```http
GET /persons/7/supervises?as=supervisor      # person 7's reports
GET /persons/7/supervises?as=report          # person 7's supervisors
```

`?as` accepts a participant name (or role name) from the `between` clause. An unknown value is `400 Bad Request`. When slots hold distinct entity types, `?as` is unnecessary and v0.1 behavior is unchanged.

### 5.4 Derived Relationship HTTP Surface

Derived relationships are **read-only associations computed by traversal** — they have no identity, no lifecycle, and no POST/PATCH/PUT/DELETE. They are query conveniences over the reified model, not new endurants. Two kinds:

#### 5.4.1 Recursive Relationships (R6)

A self-referential relationship (both participants are the same entity type) may opt-in to transitive closure queries via the `recursive` keyword.
...
The compiler generates read-only endpoints:

```http
GET /persons/7/reports-to?depth=1    # direct supervisor only
GET /persons/7/reports-to?depth=3    # up to 3 levels
GET /persons/7/reports-to?depth=*    # full transitive closure
```

The `?as=ancestor` / `?as=descendant` filter selects direction; `?depth` controls traversal depth (default: 1, unbounded: `*`).

#### 5.4.2 Chained Relationships (R7)

A chained relationship composes two or more reified relationships into a read-only association.

```eidos
chained relationship WorksForHolding {
  """Person → Employment → Organization → Ownership → Holding"""
  path {
    via Employment   from employee to employer
    via Ownership    from subsidiary to parent
  }
...
```http
GET /persons/7/works-for-holding
```

No identity, no lifecycle, GET-only.

#### 5.4.3 Response Representation

Derived relationships have no per-item identity. Each entry provides **traceability** to the underlying reified relationships:

| Field      | Description                                                                                                                |
|------------|----------------------------------------------------------------------------------------------------------------------------|
| `_via`     | Array of underlying reified relationship references, in hop order. Each has `_self` (URL) and `_type` (relationship type). |
| `endpoint` | The far-end entity's canonical URL.                                                                                        |

**Chained example** (`GET /persons/7/works-for-holding`):

```json
[
  {
    "_via": [
      { "_self": "/employments/42", "_type": "Employment" },
      { "_self": "/ownerships/99", "_type": "Ownership" }
    ],
    "endpoint": "/holdings/5"
  }
]
...
**Recursive example** (`GET /persons/7/reports-to?depth=*`):

```json
[
  {
    "_via": [
      { "_self": "/supervises/12", "_type": "Supervises" }
    ],
    "endpoint": "/persons/3",
    "depth": 1
  },
  {
    "_via": [
      { "_self": "/supervises/12", "_type": "Supervises" },
      { "_self": "/supervises/5", "_type": "Supervises" }
    ],
    "endpoint": "/persons/1",
    "depth": 2
  }
...
`?expand=endpoint` embeds the far-end entity inline.

### 5.5 Archetype Composition
...
Whether namespaces map to URL path prefixes. Cross-file entity references. Distribution of archetype libraries as packages. The `/_roles`, `?as`, and derived-relationship paths must compose with whatever namespace→path-prefix decision is made.

**7.3 Guard predicate language**
Whether guards are just names resolved externally, or whether Eidos provides a limited expression language for structural guards (e.g., property comparisons, role membership tests). The chained-relationship `where` clause (if added) must reuse this language.
...
Auto-generating `GET /{entity}/{key}/history` as an opt-in (possibly via a `History` mixin or archetype annotation).

**7.6 Pagination, filtering, sorting**
Standard query parameter conventions for collection endpoints. Recursive and chained queries can be expensive and unbounded; pagination is critical for them.
...
**7.10 Role property writability**
Whether participation (role) properties are patchable, and on which URL — the relationship's `PATCH`, or a participation-scoped path. (Leaning: via the relationship's `PATCH`, since the participation has no independent identity.)

**7.11 Chained relationship filters**
Whether chained relationships support a `where` clause for filtering mid-chain (e.g., `where employer.state in [Active]`). Depends on §7.3 guard predicate language.

**7.12 Cycle handling on derived relationship queries**
Confirm the response convention when a cycle is detected during a `depth=*` traversal: silently break it (visited set) versus surface a diagnostic.
